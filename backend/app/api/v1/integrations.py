...
class GitHubItemSummaryRequest(BaseModel):
    item_type: str
    title: str
    content: str
    metadata: str = ""
...
@router.post("/github/analyze-item")
async def analyze_github_item(
    payload: GitHubItemSummaryRequest,
    current_user: User = Depends(get_current_user),
):
    from app.services.ai_service import AIService

    try:
        summary = await AIService.analyze_github_item(
        ...
        raise HTTPException(
            status_code=500,
            detail=f"GitHub AI 분석 실패: {str(e)}"
        )
